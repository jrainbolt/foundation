...
static FactorySimulation *make_line(FactoryWorld **out_world)
{
    ...
    CHECK(factory_world_add_resource(w,0,0,FACTORY_RESOURCE_COAL,20U)==FACTORY_RESULT_OK);
...
static void test_flow_windows_and_clear(void)
{
    ...
    for(size_t tick=0U;tick<50U;++tick){CHECK(factory_simulation_tick(s)==FACTORY_RESULT_OK);
        for(size_t i=0U;i<factory_simulation_get_event_count(s);++i){const FactoryEvent*e=factory_simulation_get_event(s,i);
            if(e->type==FACTORY_EVENT_PRODUCTION_COMPLETED
                &&e->item_type==FACTORY_ITEM_COAL)
                event_produced+=e->quantity;
            if(e->type==FACTORY_EVENT_ITEM_TRANSFERRED
                &&e->item_type==FACTORY_ITEM_COAL)
                event_transferred+=e->quantity;}
    ...
    {FactoryTelemetryItemMetrics m;FactoryTelemetryEntityMetrics extractor,belt,storage;
        ...
        CHECK(factory_telemetry_get_item_metrics(t,FACTORY_ITEM_COAL,
            FACTORY_TELEMETRY_WINDOW_LONG,&m));
        ...
        CHECK(factory_telemetry_get_entity_item_metrics(t,3U,
            FACTORY_ITEM_COAL,FACTORY_TELEMETRY_WINDOW_LONG,&storage_item));
