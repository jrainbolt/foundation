...
{s->items[s->count++]=(FactoryResearchLab){id,x,y,FACTORY_ITEM_NONE,0U,
    FACTORY_RESEARCH_LAB_IDLE,0U,0U};}
    ...
    FactoryEntityId id,FactoryResearchLabInspection*out)
{
    ...
    *out=(FactoryResearchLabInspection){id,l->x,l->y,l->science_item,
        l->science_quantity,
...
void factory_research_labs_update(FactorySimulation*s)
{
    ...
    if(!p->science_committed){chosen->science_quantity-=d->science_quantity_per_unit;if(chosen->science_quantity==0U)chosen->science_item=FACTORY_ITEM_NONE;chosen->science_consumed_last_tick=d->science_quantity_per_unit;p->science_committed=true;}
