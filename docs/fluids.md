...
contains water in the aqueous class. Entity-owned `FactoryFluidStorage` state
records its owner, position, accepted class mask, current fluid, quantity, and
capacity. A storage is either empty (`NONE`, zero) or contains exactly one
fluid type. The initial fluid tank accepts aqueous fluids and holds 10,000
units.
...
snapshot version 6; pending fluid commands retain FIFO order. Loaded empty,
...
This milestone has no pipes, networks, pressure, flow rates, pumps, mixing,
temperature, chemistry, boilers, steam, or automatic production consumption.
The direct transfer command names both endpoints and is the explicit
connection for that operation.
