...
func _draw() -> void:
	...
	for x in range(left, right + 1):
		draw_line(Vector2(x * cell_size, top * cell_size), Vector2(x * cell_size, bottom * cell_size), Color("#303846", 0.72), 1.0)
	for y in range(top, bottom + 1):
		draw_line(Vector2(left * cell_size, y * cell_size), Vector2(right * cell_size, y * cell_size), Color("#303846", 0.72), 1.0)
